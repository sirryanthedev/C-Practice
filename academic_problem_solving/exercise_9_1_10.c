...
int main(){
    ...
    struct node* head_negatives = NULL;
    ...
    print_nodes(head); // (-2, -3, 4, -5)

    puts("\n");

    del_all_negatives(&head, &head_negatives);

    print_nodes(head); // positive numbers (4)

    puts("\n");

    print_nodes(head_negatives); // negative numbers (-5, -3, -2)
